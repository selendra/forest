...
use forest_test_runtime::{AccountId, Signature};
...
pub struct GenesisExt {
	...
	runtime_genesis_config: forest_test_runtime::GenesisConfig,
	/// The parachain id.
	para_id: ParaId,
}
...
impl sp_runtime::BuildStorage for GenesisExt {
	fn assimilate_storage(&self, storage: &mut sp_core::storage::Storage) -> Result<(), String> {
		sp_state_machine::BasicExternalities::execute_with_storage(storage, || {
			sp_io::storage::set(forest_test_runtime::TEST_RUNTIME_UPGRADE_KEY, &vec![1, 2, 3, 4]);
			forest_test_runtime::ParachainId::set(&self.para_id);
		});
...
pub fn local_testnet_genesis() -> forest_test_runtime::GenesisConfig {
	testnet_genesis(
...
) -> forest_test_runtime::GenesisConfig {
	forest_test_runtime::GenesisConfig {
		system: forest_test_runtime::SystemConfig {
			code: forest_test_runtime::WASM_BINARY
				.expect("WASM binary was not build, please build it!")
				.to_vec(),
			..Default::default()
		},
		parachain_system: Default::default(),
		balances: forest_test_runtime::BalancesConfig {
			balances: endowed_accounts.iter().cloned().map(|k| (k, 1 << 60)).collect(),
		},
		sudo: forest_test_runtime::SudoConfig { key: Some(root_key) },
