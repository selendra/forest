...
//! Crate used for testing with Forest.
...
use forest_client_network::BlockAnnounceValidator;
use forest_client_service::{
	prepare_node_config, start_collator, start_full_node, StartCollatorParams, StartFullNodeParams,
};
use forest_primitives_core::ParaId;
use forest_relay_chain_inprocess_interface::RelayChainInProcessInterface;
...
use forest_test_runtime::{Hash, Header, NodeBlock as Block, RuntimeApi};
...
pub use chain_spec::*;
pub use forest_test_runtime as runtime;
pub use genesis::*;
...
impl sc_executor::NativeExecutionDispatch for RuntimeExecutor {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		forest_test_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		forest_test_runtime::native_version()
	}
}
...
> {
	...
	let import_queue = forest_client_consensus_relay_chain::import_queue(
		client.clone(),
		block_import.clone(),
...
async fn build_relay_chain_interface(
	...
	let relay_chain_full_node = selendra_test_service::new_full(
		relay_chain_config,
	...
	RB: Fn(Arc<Client>) -> Result<jsonrpsee::RpcModule<()>, sc_service::Error> + Send + 'static,
{
	...
	let import_queue = forest_client_service::SharedImportQueue::new(params.import_queue);
	...
	if let Some(collator_key) = collator_key {
		let parachain_consensus: Box<dyn ParachainConsensus<Block>> = match consensus {
			Consensus::RelayChain => {
				...
				Box::new(forest_client_consensus_relay_chain::RelayChainConsensus::new(
					para_id,
					proposer_factory,
					move |_, (relay_parent, validation_data)| {
						...
						async move {
							...
							forest_primitives_parachain_inherent::ParachainInherentData::create_at(
								relay_parent,
...
/// A Forest test node instance used for testing.
pub struct TestNode {
...
impl TestNodeBuilder {
	...
	pub fn connect_to_relay_chain_node(
		mut self,
		node: &selendra_test_service::SelendraTestNode,
	) -> Self {
	...
	pub fn connect_to_relay_chain_nodes<'a>(
		mut self,
		nodes: impl IntoIterator<Item = &'a selendra_test_service::SelendraTestNode>,
	) -> Self {
	...
	pub async fn build(self) -> TestNode {
		...
		let mut relay_chain_config = selendra_test_service::node_config(
		...
		.await
		.expect("could not create Forest test service");
...
pub fn node_config(
	...
	let root = base_path.path().join(format!("forest_test_service_{}", key.to_string()));
	...
	Ok(Configuration {
		impl_name: "forest-test-node".to_string(),
...
pub fn run_relay_chain_validator_node(
...
) -> selendra_test_service::SelendraTestNode {
	let mut config = selendra_test_service::node_config(
		storage_update_func,
		tokio_handle,
		key,
		boot_nodes,
	...
	selendra_test_service::run_validator_node(
		config,
		Some(forest_test_relay_validation_worker_provider::VALIDATION_WORKER.into()),
	)
}
