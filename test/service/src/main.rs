...
use forest_primitives_core::{relay_chain::v2::CollatorPair, ParaId};
use forest_test_service::AnnounceBlockFn;
...
fn main() -> Result<(), sc_cli::Error> {
	...
	match &cli.subcommand {
		...
		Some(Subcommand::ExportGenesisState(params)) => {
			...
			let state_version = forest_test_service::runtime::VERSION.state_version();
		...
		Some(Subcommand::ExportGenesisWasm(cmd)) => {
			...
			runner.sync_run(|_config| {
				...
				let spec = forest_test_service::get_chain_spec(parachain_id);
				cmd.base.run(&spec)
			})
		},
		None => {
			...
			let consensus = cli
				...
				.then(|| {
					...
					forest_test_service::Consensus::Null
				})
				.unwrap_or(forest_test_service::Consensus::RelayChain);

			let (mut task_manager, _, _, _, _) = tokio_runtime
				.block_on(forest_test_service::start_node_impl(
					config,
					collator_key,
					selendra_config,
					parachain_id,
