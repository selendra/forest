...
pub struct TestCollatorCli {
	...
	#[command(flatten)]
	pub run: forest_client_cli::RunCmd,
...
pub struct ExportGenesisStateCommand {
	...
	#[command(flatten)]
	pub base: forest_client_cli::ExportGenesisStateCommand,
}
...
pub struct ExportGenesisWasmCommand {
	...
	#[command(flatten)]
	pub base: forest_client_cli::ExportGenesisWasmCommand,
}
...
impl SubstrateCli for TestCollatorCli {
	fn impl_name() -> String {
		"Forest zombienet test parachain".into()
	}
	...
	fn description() -> String {
		format!(
			"Forest zombienet test parachain\n\nThe command-line arguments provided first will be \
	...
	fn support_url() -> String {
		"https://github.com/selendra/forest/issues/new".into()
	}
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		Ok(match id {
			"" => Box::new(forest_test_service::get_chain_spec(ParaId::from(self.parachain_id)))
				as Box<_>,
			path => {
				let chain_spec =
					forest_test_service::chain_spec::ChainSpec::from_json_file(path.into())?;
				Box::new(chain_spec)
			},
		})
	}
	...
	fn native_runtime_version(_: &Box<dyn ChainSpec>) -> &'static RuntimeVersion {
		&forest_test_service::runtime::VERSION
	}
}
