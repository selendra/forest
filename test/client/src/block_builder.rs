...
use forest_primitives_core::{ParachainBlockData, PersistedValidationData};
use forest_primitives_parachain_inherent::{ParachainInherentData, INHERENT_IDENTIFIER};
use forest_test_relay_sproof_builder::RelayStateSproofBuilder;
use forest_test_runtime::{Block, GetLastTimestamp, Hash, Header};
...
/// An extension for the Forest test client to init a block builder.
...
impl InitBlockBuilder for Client {
	...
	fn init_block_builder_at(
		...
		let timestamp = last_timestamp + forest_test_runtime::MinimumPeriod::get();
