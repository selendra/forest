...
fn create_project(out_dir: &Path) -> PathBuf {
	...
	dependencies
		.insert("forest-test-relay-validation-worker-provider".into(), dependency_project.into());
	...
	fs::write(
		...
		r#"
			forest_test_relay_validation_worker_provider::selendra_node_core_pvf::decl_puppet_worker_main!();
		"#,
	)
