...
use forest_relay_chain_inprocess_interface::{check_block_in_chain, BlockCheckStatus};
use forest_relay_chain_interface::{RelayChainError, RelayChainResult};
use forest_test_service::runtime::{Block, Hash, Header};
		...
		if self.data.lock().has_pending_availability {
			Ok(Some(CommittedCandidateReceipt {
				descriptor: CandidateDescriptor {
					para_head: selendra_parachain::primitives::HeadData(default_header().encode())
						.hash(),
					para_id: 0u32.into(),
...
async fn make_gossip_message_and_header(
	...
	let candidate_receipt = CommittedCandidateReceipt {
		...
		descriptor: CandidateDescriptor {
			...
			erasure_root: PHash::random(),
			signature: sp_core::sr25519::Signature([0u8; 64]).into(),
			para_head: selendra_parachain::primitives::HeadData(header.encode()).hash(),
			validation_code_hash: ValidationCodeHash::from(PHash::random()),
		},
	};
...
fn relay_parent_not_imported_when_block_announce_is_processed() {
	block_on(async move {
		...
		let block = client.init_selendra_block_builder().build().expect("Build new block").block;
