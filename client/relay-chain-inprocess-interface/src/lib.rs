...
///
/// The builder takes a [`selendra_client::Client`]
/// that wraps a concrete instance. By using [`selendra_client::ExecuteWithClient`]
...
struct RelayChainInProcessInterfaceBuilder {
	selendra_client: selendra_client::Client,
	backend: Arc<FullBackend>,
	sync_oracle: Arc<dyn SyncOracle + Send + Sync>,
	overseer_handle: Handle,
}
...
impl RelayChainInProcessInterfaceBuilder {
	pub fn build(self) -> Arc<dyn RelayChainInterface> {
		self.selendra_client.clone().execute_with(self)
	}
}
...
/// Build the Selendra full node using the given `config`.
#[sc_tracing::logging::prefix_logs_with("Relaychain")]
fn build_selendra_full_node(
...
pub fn build_inprocess_relay_chain(
	mut selendra_config: Configuration,
	parachain_config: &Configuration,
	telemetry_worker_handle: Option<TelemetryWorkerHandle>,
	...
	// to the telemetry.
	selendra_config.impl_version = selendra_cli::Cli::impl_version();
	selendra_config.impl_name = selendra_cli::Cli::impl_name();

	let (full_node, collator_key) = build_selendra_full_node(
		selendra_config,
		parachain_config,
		telemetry_worker_handle,
		hwbench,
	...
	let relay_chain_interface_builder = RelayChainInProcessInterfaceBuilder {
		selendra_client: full_node.client.clone(),
...
mod tests {
	...
	fn do_not_resolve_after_different_block_import_notification_was_received() {
		...
		block_builder.push_selendra_extrinsic(ext).expect("Push extrinsic");
