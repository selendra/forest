...
//! Forest Collator implementation for Substrate.

use forest_client_network::WaitToAnnounce;
...
use selendra_primitives::v2::{CollatorPair, Id as ParaId};
...
/// The logging target.
const LOG_TARGET: &str = "forest-collator";

/// The implementation of the Forest `Collator`.
	...
	RA::Api: CollectCollationInfo<Block>,
{
	...
	async fn produce_candidate(
			...
			.into_compact_proof::<HashFor<Block>>(last_head.state_root().clone())
		{
			Ok(proof) => proof,
			Err(e) => {
				tracing::error!(target: "forest-collator", "Failed to compact proof: {:?}", e);
				return None
			},
		};
...
mod tests {
	...
	use forest_client_consensus_common::ParachainCandidate;
	use forest_test_client::{
		Client, ClientBlockImportExt, DefaultTestClientBuilderExt, InitBlockBuilder,
		TestClientBuilder, TestClientBuilderExt,
	};
	use forest_test_runtime::{Block, Header};
