...
use forest_test_service::{initial_head_data, Keyring::*};
use futures::join;
use std::sync::Arc;
...
async fn pov_recovery() {
	...
	let alice = forest_test_service::run_relay_chain_validator_node(
		tokio_handle.clone(),
		Alice,
		|| {},
		Vec::new(),
	...
	// Start bob
	let bob = forest_test_service::run_relay_chain_validator_node(
		tokio_handle.clone(),
		Bob,
		|| {},
	...
	alice
		.register_parachain(
			para_id,
			forest_test_service::runtime::WASM_BINARY
	...
	let charlie =
		forest_test_service::TestNodeBuilder::new(para_id, tokio_handle.clone(), Charlie)
			.enable_collator()
	...
	let dave = forest_test_service::TestNodeBuilder::new(para_id, tokio_handle.clone(), Dave)
		.enable_collator()
		.use_null_consensus()
	...
	let eve = forest_test_service::TestNodeBuilder::new(para_id, tokio_handle.clone(), Eve)
		.use_null_consensus()
		.connect_to_parachain_node(&charlie)
		.connect_to_relay_chain_nodes(vec![&alice, &bob])
		.wrap_announce_block(|_| {
	...
	//
	// They will need to recover the pov blocks through availability recovery.
	let ferdie = forest_test_service::TestNodeBuilder::new(para_id, tokio_handle.clone(), Ferdie)
		.use_null_consensus()
		.connect_to_parachain_node(&charlie)
		.connect_to_relay_chain_nodes(vec![&alice, &bob])
	...
	let one = forest_test_service::TestNodeBuilder::new(para_id, tokio_handle, One)
		.enable_collator()
		.use_null_consensus()
		.connect_to_parachain_node(&charlie)
		.connect_to_relay_chain_nodes(vec![&alice, &bob])
