...
use forest_relay_chain_interface::{RelayChainInterface, RelayChainResult};
use sc_client_api::{
...
const LOG_TARGET: &str = "forest-consensus";
	...
	B: Backend<Block>,
{
	...
	loop {
		let finalized_head = if let Some(h) = finalized_heads.next().await {
			...
			tracing::debug!(target: "forest-consensus", "Stopping following finalized head.");
			return
		};
