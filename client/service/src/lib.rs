...
use forest_client_consensus_common::ParachainConsensus;
use forest_primitives_core::{CollectCollationInfo, ParaId};
...
{
	let consensus = forest_client_consensus_common::run_parachain_consensus(
		para_id,
		client.clone(),
	...
	task_manager
		...
		.spawn("forest-consensus", None, consensus);

	let overseer_handle = relay_chain_interface
		.overseer_handle()
		.map_err(|e| sc_service::Error::Application(Box::new(e)))?;

	let pov_recovery = forest_client_pov_recovery::PoVRecovery::new(
		overseer_handle.clone(),
		// We want that collators wait at maximum the relay chain slot duration before starting
		// to recover blocks.
		forest_client_pov_recovery::RecoveryDelay::WithMax { max: relay_chain_slot_duration },
		client.clone(),
		import_queue,
		relay_chain_interface.clone(),
		para_id,
	...
	task_manager
		.spawn_essential_handle()
		.spawn("forest-pov-recovery", None, pov_recovery.run());
	forest_client_collator::start_collator(forest_client_collator::StartCollatorParams {
		runtime_api: client,
		block_status,
		announce_block,
		overseer_handle,
	...
	IQ: ImportQueue<Block> + 'static,
{
	...
	let pov_recovery = forest_client_pov_recovery::PoVRecovery::new(
		...
		// for block recovery.
		forest_client_pov_recovery::RecoveryDelay::WithMinAndMax {
