...
fn crate_() -> Result<Ident, Error> {
	match crate_name("forest-pallet-parachain-system") {
		Ok(FoundCrate::Itself) =>
			Ok(syn::Ident::new("forest_pallet_parachain_system", Span::call_site())),
		...
		quote::quote! {
			...
			mod parachain_validate_block {
				...
				unsafe fn validate_block(arguments: *const u8, arguments_len: usize) -> u64 {
					let params = #crate_::validate_block::selendra_parachain::load_params(
						arguments,
						arguments_len,
					);
					...
					#crate_::validate_block::selendra_parachain::write_result(&res)
				}
			}
		}
	} else {
