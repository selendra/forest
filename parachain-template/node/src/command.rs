		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				if collator_options.relay_chain_rpc_url.is_some() && cli.relay_chain_args.len() > 0 {
					warn!("Detected relay chain node arguments together with --relay-chain-rpc-url. This command starts a minimal Selendra node that only uses a network-related subset of all relay chain CLI options.");
				}
